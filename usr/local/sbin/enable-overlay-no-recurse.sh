...
if grep '/boot/firmware' /proc/mounts | grep -q "[[:space:]]rw[[:space:],]" || [ "$1" == "force-remount-rw" ] ; then
	...
	if raspi-config nonint enable_overlayfs && \
	   sed -e 's#overlayroot=tmpfs#overlayroot=tmpfs:recurse=0#' -i /boot/firmware/cmdline.txt ; then
		echo "Non-Recursing Overlayroot will be activated upon next reboot."
